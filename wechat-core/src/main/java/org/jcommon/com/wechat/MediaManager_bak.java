...
public class MediaManager_bak extends ResponseHandler{
    ...
    public MediaManager_bak(WechatSession session){
    ...
    public HttpRequest downloadMedia(InMessage message){
         ...
         FileRequest request = (FileRequest)RequestFactory.downloadMediaRequest(this, app.getAccess_token(), message.getMediaId(), file);
         request.setHandler(message);
         addHandlerObject(request, Media.class);
         session.execute(request);
         return request;

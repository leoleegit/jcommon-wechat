...
public class MsgManager extends ResponseHandler implements MediaManagerListener {
	...
	public HttpRequest sendNews(String touser, News news,
			RequestCallback callback) {
		return sendNews(touser, news, null, callback);
	}

	public HttpRequest sendNews(String touser, News news,
			CustomService customservice, RequestCallback callback) {
	...
	public HttpRequest sendText(String touser, Text text,
			RequestCallback callback) {
	...
	public HttpRequest sendImage(String touser, Image image,
			RequestCallback callback) {
	...
	public HttpRequest sendVideo(String touser, Video video,
			RequestCallback callback) {
	...
	public HttpRequest sendMusic(String touser, Music music,
			CustomService customservice, RequestCallback callback) {
		OutMessage msg = new OutMessage(MsgType.video, touser);
		msg.setMusic(music);
		msg.setCustomservice(customservice);
	...
	private HttpRequest sendMsg(OutMessage msg, RequestCallback callback,
		...
		HttpRequest request = RequestFactory.msgReqeust(callback, session
				.getApp().getAccess_token(), msg.toJson());
		if (execute)
			session.execute(request);
		return request;
	}

	public HttpRequest broadcastMpnews(Filter filter, Mpnews mpnews,
			RequestCallback callback) {
	...
	public HttpRequest broadcastText(Filter filter, Text text,
			RequestCallback callback) {
	...
	public HttpRequest broadcastVideo(Filter filter, Video video,
			RequestCallback callback) {
		BroadcastMessage msg = new BroadcastMessage(MsgType.video, filter);
		msg.setVideo(video);
		if (video.getMedia_id() == null) {
			HttpRequest upload = media_manager.uploadMedia(video, this);
			upload.setAttribute(request_object,
					broadcastMsg(msg, callback, false));
			upload.setAttribute(message_object, msg);
			return upload;
		}
		return broadcastMsg(msg, callback);
	...
	public HttpRequest broadcastImage(Filter filter, Image image,
			RequestCallback callback) {
	...
	private HttpRequest broadcastMsg(OutMessage msg, RequestCallback callback,
		...
		HttpRequest request = RequestFactory.broadcastReqeust(callback, session
				.getApp().getAccess_token(), msg.toJson());
		if (execute)
			session.execute(request);
		return request;

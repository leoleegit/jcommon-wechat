...
public class QrcodeManager extends ResponseHandler{
	...
	public void onOk(HttpRequest paramHttpRequest, Object paramObject) {
		// TODO Auto-generated method stub
		if(paramHttpRequest.getAttibute(paramHttpRequest)!=null){
			QrcodeManagerListener listener = (QrcodeManagerListener) paramHttpRequest.getAttibute(paramHttpRequest);
			if(listener!=null){
	...
	public HttpRequest createQrcode(ActionInfo info, QrcodeManagerListener listener){
		if(info==null){
			logger.warn("info is null");
			return null;
		}

		HttpRequest request = RequestFactory.createQrcodeReqeust(this, session.getApp().getAccess_token(),info.toJson());
		if(listener!=null)

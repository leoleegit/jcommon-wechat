...
public class WechatSessionManager extends Monitor implements MapStoreListener {
	...
	public void initOperation() {
		...
		addOperation(new MBeanOperationInfo("removeSession", "removeSession",
				new MBeanParameterInfo[] { new MBeanParameterInfo("wechatID",
						"java.lang.String", "wechatID") }, // no parameters
				"void", MBeanOperationInfo.ACTION));
		addOperation(new MBeanOperationInfo("restartSession", "restartSession",
				new MBeanParameterInfo[] { new MBeanParameterInfo("wechatID",
						"java.lang.String", "wechatID") }, // no parameters
				"void", MBeanOperationInfo.ACTION));
		addOperation(new MBeanOperationInfo("addSession", "addSession",
				new MBeanParameterInfo[] {
						new MBeanParameterInfo("wechatID", "java.lang.String",
								"wechatID"),
						new MBeanParameterInfo("appid", "java.lang.String",
								"appid"),
						new MBeanParameterInfo("secret", "java.lang.String",
								"secret"),
						new MBeanParameterInfo("Token", "java.lang.String",
								"Token") }, // no parameters
				"void", MBeanOperationInfo.ACTION));
	}
	...
	public void addSession(String wechatID, String appid, String secret,
			String Token) {
		logger.info(String.format(
				"wechatID:%s ; appid:%s ; secret:%s ; Toke:%s ;", wechatID,
				appid, secret, Token));
		...
		if (session == null) {
			...
			logger.info(String.format("session of %s have been exist!",
					wechatID));
		}
	}
	...
	private void addSessioin(WechatSession session) {
		...
		String value = session.getApp() != null ? session.getApp().toJson()
				: "app is null";
	...
	public void onToken(String signature, String timestamp, String nonce,
			String xml) {
		this.logger.info(xml);
		Token token = new Token(xml, signature, timestamp, nonce);
		String touser = token.getWechatID();
		boolean done = false;
		List<WechatSession> sessions = SessionCache.instance()
				.getAllWechatSession();
		for (WechatSession session : sessions) {
			if ((touser != null && touser.equals(session.getWechatID()))
					|| "*".equals(session.getWechatID())) {
	...
	public void onCallback(String encrypt_type, String msg_signature,
			String signature, String timestamp, String nonce, String xml) {
		this.logger.info(xml);
		...
		if (root.element("Encrypt") != null) {
			Encrypt encrypt = new Encrypt(encrypt_type, msg_signature,
	...
	public void onCallback(String signature, String timestamp, String nonce,
			String xml) {
		this.logger.info(xml);
		Document document = null;
		Element root = null;

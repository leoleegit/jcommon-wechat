...
public class WechatSession implements WechatSessionListener,
		MediaManagerListener, UserManagerListener {
	...
	public WechatSession(String wechatID, App app,
	...
	public WechatSession(WXBizMsgCrypt msg_crypt, String wechatID, App app,
			WechatSessionListener listener) {
	...
	public boolean appVerify(String signature, String timestamp, String nonce) {
		...
		if (token != null && signature != null && timestamp != null
				&& nonce != null) {
			return signature.equalsIgnoreCase(WechatUtils.createSignature(
	...
	public String onEncrypt(Encrypt encrypt) throws AesException {
		...
		logger.info(logStr("msg_signature:%s;timestamp:%s;nonce:%s",
				encrypt.getMsg_signature(), encrypt.getTimestamp(),
				encrypt.getNonce()));
		if (getMsg_crypt() != null)
			return getMsg_crypt().decryptMsg(encrypt.getMsg_signature(),
					encrypt.getTimestamp(), encrypt.getNonce(),
					encrypt.getXml());
		return null;
	}

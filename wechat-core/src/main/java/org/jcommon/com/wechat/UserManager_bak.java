...
public class UserManager_bak extends ResponseHandler implements SystemListener{
	...
	public UserManager_bak(WechatSession session){
	...
	public void loadUsers(String next_openid){
		HttpRequest request = RequestFactory.getUsersReqeust(this, app.getAccess_token(), next_openid);
	...
	public HttpRequest getUser(InMessage message){
		...
		HttpRequest request = RequestFactory.userInfoRequest(this, app.getAccess_token(), message.getFromUserName(), null);
	...
	private void loadGroups(){
		load_groups = null;
		HttpRequest request = RequestFactory.getGroupsReqeust(this,this.app.getAccess_token());
	...
	public void onError(final HttpRequest request, Error paramError) {
		...
		if("loadGroups".equals(request_action)){
			load_groups = org.jcommon.com.util.thread.TimerTaskManger.instance().schedule("loadGroups", 
					new TimerTask(){
						public void run(){
							UserManager_bak.this.loadGroups();
						}
			}, 20000);
			...
			load_users = org.jcommon.com.util.thread.TimerTaskManger.instance().schedule("load_users", 
					new TimerTask(){
						public void run(){
							...
							UserManager_bak.this.loadUsers(next_openid);
						}
			}, 20000);

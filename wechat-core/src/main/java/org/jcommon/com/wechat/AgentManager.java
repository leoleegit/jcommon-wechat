...
public class AgentManager extends ResponseHandler {
	...
	public void onError(HttpRequest paramHttpRequest, Error paramError) {
		...
		if (paramHttpRequest.getAttibute(paramHttpRequest) != null) {
			AgentManagerListener listener = (AgentManagerListener) paramHttpRequest
	...
	public HttpRequest getAgent(AgentManagerListener listener) {
		HttpRequest request = RequestFactory.getAgentRequest(this, session
	...
	public HttpRequest addAgent(Agent agent, RequestCallback callback) {
		HttpRequest request = RequestFactory.addAgentRequest(callback, session
				.getApp().getAccess_token(), agent.toJson());
		logger.info(request.getUrl());
		session.execute(request);
		return request;
	}
	...
	public HttpRequest updateAgent(Agent agent, RequestCallback callback) {
		HttpRequest request = RequestFactory.updateAgentRequest(callback,
				session.getApp().getAccess_token(), agent.toJson());
		logger.info(request.getUrl());
		session.execute(request);
		return request;
	}
	...
	public HttpRequest delAgent(Agent agent, RequestCallback callback) {
		HttpRequest request = RequestFactory.delAgentRequest(callback, session
				.getApp().getAccess_token(), agent.toJson());
		logger.info(request.getUrl());
		session.execute(request);
		return request;
	...
	public HttpRequest uploadAgentHeadImg(Agent agent, RequestCallback callback) {
		HttpRequest request = RequestFactory.uploadHeadImgRequest(callback,
				session.getApp().getAccess_token(), agent.getMedia(),
				agent.getKf_account());
		logger.info(request.getUrl());
		session.execute(request);
		return request;

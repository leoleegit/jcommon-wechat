...
public class MenusManager extends ResponseHandler{
	...
	public void onOk(HttpRequest paramHttpRequest, Object paramObject) {
		// TODO Auto-generated method stub
		if(paramHttpRequest.getAttibute(paramHttpRequest)!=null){
			MenusManagerListener listener = (MenusManagerListener) paramHttpRequest.getAttibute(paramHttpRequest);
			if(listener!=null){
	...
	public HttpRequest getMenus(MenusManagerListener listener){
		HttpRequest request = RequestFactory.getMenusReqeust(this, session.getApp().getAccess_token());
		if(listener!=null)
	...
	public HttpRequest delMenus(RequestCallback callback){
		HttpRequest request = RequestFactory.delMenusReqeust(callback, session.getApp().getAccess_token());
		session.execute(request);
		return request;
	}
	
	public HttpRequest createMenus(Menus menus, RequestCallback callback){
		HttpRequest request = RequestFactory.createMenusReqeust(callback, session.getApp().getAccess_token(),menus.toJson());
		session.execute(request);
		return request;
	}
}

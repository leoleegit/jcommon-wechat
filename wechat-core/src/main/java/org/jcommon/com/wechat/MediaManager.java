...
public class MediaManager extends ResponseHandler {
	...
	public void onOk(HttpRequest paramHttpRequest, Object paramObject) {
		...
		if (paramHttpRequest instanceof FileRequest) {
			...
			if (paramHttpRequest.getAttibute(paramHttpRequest) != null) {
				MediaManagerListener listener = (MediaManagerListener) paramHttpRequest
						.getAttibute(paramHttpRequest);
				Media media = (Media) paramHttpRequest
						.getAttibute(media_object);
				...
				if (listener != null) {
					if (paramObject instanceof Media) {
						...
						listener.onMedia(paramHttpRequest, getMedia_factory()
								.createUrl(media));
					}
				}
			...
			if (paramHttpRequest.getAttibute(paramHttpRequest) != null) {
				...
				if (listener != null) {
					if (paramObject instanceof MaterialsCount) {
						...
						listener.onMaterialsCount(paramHttpRequest,
								media_return);
	...
	public HttpRequest uploadNews(Mpnews mpnews, MediaManagerListener listener) {
		HttpRequest request = RequestFactory.uploadNewsRequest(this, session
				.getApp().getAccess_token(), mpnews.toJson());
		if (listener != null)
	...
	public HttpRequest getMaterials(String type, int offset, int count,
			MediaManagerListener listener) {
		if (count <= 0 || count > 20)
			count = 20;

		HttpRequest request = RequestFactory.getMaterialsReqeust(this, session
				.getApp().getAccess_token(), new Materials(type, offset, count)
				.toJson());
		if (listener != null)
			request.setAttribute(request, listener);
	...
	public HttpRequest getMaterial(Media media, MediaManagerListener listener) {
		...
		HttpRequest request = RequestFactory.getMaterialReqeust(this, session
				.getApp().getAccess_token(), media.toJson(), null);
		if (listener != null)
			request.setAttribute(request, listener);
	...
	public HttpRequest getMaterialCount(MediaManagerListener listener) {
		HttpRequest request = RequestFactory.getMaterialCountReqeust(this,
	...
	public HttpRequest delMaterial(Media media, RequestCallback callback) {
		...
		HttpRequest request = RequestFactory.delMaterialReqeust(callback,
				session.getApp().getAccess_token(), media.toJson());
		session.execute(request);
		return request;
	}

	public HttpRequest updateMaterial(Articles articles, int index,
		...
		HttpRequest request = RequestFactory.updateMaterialReqeust(callback,
				session.getApp().getAccess_token(), out.toJson());
		session.execute(request);
		return request;
	}
	...
	public HttpRequest downloadMedia(Media media, MediaManagerListener listener) {
		...
		HttpRequest request = RequestFactory.downloadMediaRequest(this, session
				.getApp().getAccess_token(), media_id, file);
		if (listener != null)
			request.setAttribute(request, listener);
	...
	public HttpRequest uploadMedia(Media media, MediaManagerListener listener) {
		...
		if (file == null)
			file = getMedia_factory().getMediaFromUrl(media.getUrl())
					.getMedia();
		if (file == null) {
		...
		HttpRequest request = RequestFactory.uploadMediaRequest(this, session
				.getApp().getAccess_token(), file, media_type);
		if (listener != null)
			request.setAttribute(request, listener);
		request.setAttribute(media_object, media);
		super.addHandlerObject(request, Media.class);
	...
	public HttpRequest uploadMaterialMedia(Media media, String description,
			MediaManagerListener listener) {
		if (media == null || media.getType() == null) {
			logger.warn("media or media type is null");
			return null;
		...
		String media_type = media.getType();
		HttpRequest request = RequestFactory.uploadMaterialMediaRequest(this,
				session.getApp().getAccess_token(), file, media_type,
				description);
		if (listener != null)
			request.setAttribute(request, listener);
		request.setAttribute(media_object, media);
	...
	public HttpRequest uploadMaterialMedia(Image image,
			MediaManagerListener listener) {
		return uploadMaterialMedia(image, null, listener);
	}

	public HttpRequest uploadMaterialMedia(Voice voice,
			MediaManagerListener listener) {
		return uploadMaterialMedia(voice, null, listener);
	}

	public HttpRequest uploadMaterialMedia(Thumb thumb,
			MediaManagerListener listener) {
		return uploadMaterialMedia(thumb, null, listener);
	}

	public HttpRequest uploadMaterialMedia(Video video,
			MediaManagerListener listener) {
		if (video == null) {
	...
	public HttpRequest uploadNewsImg(Media media, MediaManagerListener listener) {
		...
		HttpRequest request = RequestFactory.uploadImgRequest(this, session
				.getApp().getAccess_token(), file);
		if (listener != null)
			request.setAttribute(request, listener);
		request.setAttribute(media_object, media);
		super.addHandlerObject(request, Media.class);
